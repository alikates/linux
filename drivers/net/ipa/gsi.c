 ...
 */
static bool gsi_command(struct ipa_dma *gsi, u32 reg, u32 val)
{
...
void gsi_trans_tx_committed(struct ipa_trans *trans)
{
...
void gsi_trans_tx_queued(struct ipa_trans *trans)
{
	u32 channel_id = trans->channel_id;
	struct ipa_dma *gsi = trans->dma_subsys;
 ...
 */
static void gsi_trans_tx_completed(struct ipa_trans *trans)
{
	u32 channel_id = trans->channel_id;
	struct ipa_dma *gsi = trans->dma_subsys;
	struct ipa_channel *channel;
...
static struct ipa_trans *
gsi_event_trans(struct ipa_dma *gsi, struct gsi_event *event)
{
	...
	channel = &gsi->channel[channel_id];
	if (WARN(!channel->dma_subsys, "event has bad channel %u\n", channel_id))
		return NULL;
 ...
 */
static void gsi_evt_ring_update(struct ipa_dma *gsi, u32 evt_ring_id, u32 index)
{
	struct gsi_evt_ring *evt_ring = &gsi->evt_ring[evt_ring_id];
	struct gsi_ring *ring = &evt_ring->ring;
...
void
gsi_modem_channel_flow_control(struct ipa_dma *gsi, u32 channel_id, bool enable)
{
	u32 retries = 0;
	u32 command;
	int ret;
...
static bool gsi_channel_data_valid(struct ipa_dma *gsi, bool command,
