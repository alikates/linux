...
struct ipa_dma {
	...
	u32 (*channel_trans_idle)(struct ipa_dma *dma_subsys, u32 channel_id);
...
/**
 * ipa_channel_trans_idle() - Return true if no transactions are allocated, false otherwise 
 * @dma_subsys:	pointer to ipa_dma structure
 * @channel_id:	Channel
 *
 * Return: Return true if no transactions are allocated, false otherwise 
 */
static inline u32 ipa_channel_trans_idle(struct ipa_dma *dma_subsys, u32 channel_id)
{
	return dma_subsys->channel_trans_idle(dma_subsys, channel_id);
}
 ...
 */
void gsi_modem_channel_flow_control(struct ipa_dma *dma_subsys, u32 channel_id,
				    bool enable);
...
/**
 * ipa_dma_suspend() - Prepare the DMA subsystem for suspend
 * @dma_subsys:	ointer to ipa_dma structure
 */
...
/**
 * ipa_dma_resume() - Resume the DMA subsystem following suspend
 * @dma_subsys:	ointer to ipa_dma structure
 */
