...
static void gsi_trans_map(struct ipa_trans *trans, u32 index)
{
...
static void gsi_trans_move_committed(struct ipa_trans *trans)
{
	struct ipa_channel *channel = &trans->dma_subsys->channel[trans->channel_id];
...
void ipa_trans_free(struct ipa_trans *trans)
{
	 ...
	 */
	ipa_trans_tre_release(trans_info, trans->rsvd_count);
}
...
int ipa_trans_skb_add(struct ipa_trans *trans, struct sk_buff *skb)
{
	...
	ret = dma_map_sg(trans->dma_subsys->dev, sg, used_count, trans->direction);
	if (!ret)
		return -ENOMEM;
...
void ipa_trans_complete(struct ipa_trans *trans)
{
	...
	if (trans->direction != DMA_NONE)
		dma_unmap_sg(trans->dma_subsys->dev, trans->sgl, trans->used_count,
			     trans->direction);
